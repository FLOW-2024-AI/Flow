...
const proxyPath = '/facturas-pendientes';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error proxying GET /facturas-pendientes:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error proxying POST /facturas-pendientes:', error);
    return NextResponse.json({ success: false, error: serviceError }, { status: 502 });
  }
}
